...
**Hosted ability definition integrity:** Signing and drift-monitoring above assume an ability is a component the system holds. A remote or third-party MCP server (or hosted skill) instead serves its tool definitions — name, description, and input schema — at connection time, and can change them after a client has approved them without altering anything the client signed (see **Tool integration vulnerabilities** above and [Agentic AI threats](/go/agenticaithreats)). Pin the approved definition, re-verify it on fetch or on a defined cadence, and treat an unreviewed change as a supply-chain change requiring re-review rather than silent acceptance. Because a tool description is passed to the model as instruction, an altered description is also an [indirect prompt injection](/go/indirectpromptinjection) vector even if the tool is never called; review and constrain third-party descriptions and schemas as untrusted model-facing input. Config-swap and mutable-definition ("rug-pull") variants have been reported in the wild and described in the literature (e.g. CVE-2025-54136; ETDI, arXiv:2506.01333).
