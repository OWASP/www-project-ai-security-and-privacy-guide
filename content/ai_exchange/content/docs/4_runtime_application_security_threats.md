...
(runtime appsec). Run-time model confidentiality: see [SECDEVPROGRAM](/goto/c4/) to attain application security, with the focus on protecting the storage of model parameters (e.g. access control, encryption).  
...
(runtime appsec). Model input confidentiality: see [SECDEVPROGRAM](/goto/c4/) to attain application security, with the focus on protecting the transport and storage of model input (e.g. access control, encryption, minimize retention)
