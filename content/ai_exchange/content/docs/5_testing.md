...
- **Identify Potential Threats:** Threat modeling, identification of attack surface, exploration, and threat actors. A threat model records the assumptions the test rests on: the attacker's goals, their knowledge of the system, and their capabilities, including how they can reach the system and what resources and budget they can bring.
...
NOTE: This procedure targets models that perform a classification, detection, or regression task. That includes predictive AI models, and it also includes generative AI models used for such a task, for example moderating content or triaging messages. Generative AI models can also be evaded through optimized inputs such as adversarial suffixes or perturbed images. Evasion and prompt injection often appear together in the same system and are tested differently, since evasion manipulates the data the model works on while prompt injection supplies instructions. See [Evasion](/go/evasion) for how the two relate, and [prompt injection testing](/go/testingpromptinjection) for instruction-based attacks.

#### Test procedure
See the [section above](/go/testing) for the general steps in AI security testing.  
The steps specific for testing against this threat are:

**(1) Threat model and evasion input feasibility**  
First, record the threat model that the test represents: what the attacker knows about the model (parameters, architecture, or output only), how they reach it (a local copy, an API, or an application in front of it), whether they aim for one specific wrong output or any wrong output, how many queries they can afford, and what resources the relevant attacker profile brings, such as the ability to obtain or train a model for a similar task. The attack types in step 2 follow from these choices, so a reviewer can see why each one was run and why others were left out.

Then establish criteria for deciding whether evasion inputs are feasible. These criteria include:
...
The perturbation limit needs a justification that names the constraint it stands for. Two different constraints can set it. What the attacker is able to change at all is part of the threat model above and is stated as an assumption. How much change gets past a detector, a human reviewer, or another oversight mechanism is a property of the deployed system, and can only be established by measuring it, which is what step 3 is for. Where both apply, the tighter of the two governs.

A limit adopted unchanged from a robustness benchmark answers neither question. Benchmark thresholds are selected so that the true label stays the same for inputs inside the perturbation set, which is a label-preservation criterion rather than a detectability one. The two come apart in practice: in a study of 330 observers judging images at 224x224 resolution, success at identifying images as perturbed reached over 80% at an L-infinity budget of 8/255, a value in common benchmark use. Robustness within an Lp limit is therefore a lower bound rather than a guarantee. Failing it is evidence of weakness, while passing it says nothing about inputs that fall outside the limit but remain feasible for the attacker.
...
- When the attacker profile includes the ability to obtain or train a model for the same or a similar task, perform a **transfer attack** using surrogate models. Follow the procedure of the first step but search against the surrogate model. A transfer attack is how an attacker without access to the model parameters can still use gradients, which makes it the relevant test where the model is only reachable as a black box. It is also the search that [#RATE LIMIT](/go/ratelimit), [#MODEL ACCESS CONTROL](/go/modelaccesscontrol), and [#OBSCURE CONFIDENCE](/go/obscureconfidence) do not constrain, because it is performed on a model the attacker controls. Being able to build a surrogate is not on its own a reason to run this step: it is run when the relevant attacker profile has that capability. The surrogate may be a commercial or open-source model, or a new model trained on task data that the attacker can obtain or create. This step is not required if earlier steps already found a feasible evasion input with unacceptable results.
- Where the threat model allows the attacker to manipulate the input, perform a **heuristic attack** on the model, also called domain-informed or manual adversarial testing. This applies when domain knowledge and understanding of the intended use suggest that certain changes may produce the desired effect. For example, testers may check whether an image of a red balloon on a sidewalk is classified as a stop sign. Unlike the categories above, it draws on domain expertise rather than an optimization algorithm, which makes it a complementary test rather than an alternative to the searches above. Finding one successful evasion input does not establish how broadly or how easily the model can be misled, and a heuristic attack can surface a weakness that an attacker could exploit without any knowledge of the model internals. For that reason it is still performed where an earlier step already found a feasible evasion input.
...
NOTE: Gradient-based searches such as PGD and Auto-PGD can fail for the wrong reason. Some defenses break the gradient signal instead of the underlying weakness, which is known as gradient masking or obfuscated gradients. Athalye et al. found this in 7 of the 9 non-certified white-box defenses published at ICLR 2018. Three checks detect it:
...
EXAMPLE: Known methods for searching adversarial examples at the time of writing include Auto-PGD and the Fast Adaptive Boundary attack for perfect-knowledge search, Square Attack for zero-knowledge search, and HopSkipJump where only labels are returned. AutoAttack bundles the first two with Square Attack into a single evaluation. These are available through several of the tools in our [test tools section](/go/testingtoolspredictiveai), including ART and Foolbox.
...
EXAMPLE: An AI system reads dental X-rays and reports findings such as "possible proximal caries on tooth #19". A tester applies domain-plausible variations of the kind that could arise during image acquisition, processing, or handling, leaving the clinical content unchanged, and submits the result through the normal path. The original image produces "possible caries on #19" and the modified image produces "no evidence of caries". Where the modification stays perceptually similar to the original and passes the system's input controls, but changes the interpretation in a clinically meaningful way, that is a heuristic evasion finding. The question being answered is not whether an adversarial example exists somewhere, but whether realistic and relatively simple manipulations can change the output in ways that matter.
...
- [Redgrave & Czajka, "Salient Information Preserving Adversarial Training Improves Clean and Robust Accuracy", 2025](https://arxiv.org/abs/2501.09086) - includes a 330-participant study of human detection of perturbed images
...
NOTE: DEEPSEC is the reference implementation for the IEEE S&P 2019 paper by Ling et al. It has had no commits since May 2019, has a single contributor, and targets PyTorch 0.4, so adopting it carries the operational risk of depending on unmaintained code with no route to fixes or security updates. Its published results have also been disputed: [Carlini (2019)](https://arxiv.org/abs/1905.07112) reports that DEEPSEC evaluates each defense obliviously, using attacks crafted against undefended models, and uses attack implementations that greatly under-estimate their effectiveness. That is the evaluation error the adaptive attack guidance in [testing against evasion](/go/testingevasion) is intended to prevent. Treat DEEPSEC as a research artifact for reproducing the original paper rather than a tool for current testing. ART and Foolbox cover the same attacks and are maintained.
...
|  | - **Trend:** No development activity since May 2019 |
| **Community Support** | - **Active Issues:** ~15 open, including reported defects in attack implementations; last maintainer response July 2019. |
|  | - **Documentation:** Available through GitHub, covering setup, use, and contributions. |
|  | - **Discussion Forums:** GitHub Discussions is not enabled for this repository. |
|  | - **Contributors:** One contributor. |
