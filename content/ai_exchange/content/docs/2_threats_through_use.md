...
Recognizing new or unseen data that differs from the training data but might still be relevant or of interest. Novelty detection is often related to OOD but focuses more on identifying new, relevant instances rather than just any data that is different.
...
Classifying known classes while identifying and rejecting unknown classes during testing. OSR is closely related to OOD, as it involves recognizing when an instance does not belong to any of the learned categories.
