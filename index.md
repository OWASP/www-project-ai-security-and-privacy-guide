...
And the [Application Security podcast](https://www.buzzsprout.com/1730684/12313155-rob-van-der-veer-owasp-ai-security-privacy-guide) about this guide.
...
* **AI model attacks**, which all require machine learning knowledge and not application security expertise per se - see also [BIML](https://berryvilleiml.com/taxonomy/), [ENISA](https://www.enisa.europa.eu/publications/securing-machine-learning-algorithms), [ETSI SAI Problem statement Section 6](https://www.etsi.org/committee/1640-sai#), and [Microsoft](https://docs.microsoft.com/en-us/security/failure-modes-in-machine-learning). These are often referred to as _adversarial_ machine learning attacks:
  ...
  * **Input manipulation attack**: fooling models with deceptive input data. This attack can be done in three ways: 1) by experimenting with the model input (black box), 2) by introducing maliciously designed data based on analysis of the model parameters (white box), and 3) by basing the input on data poisoning that took place (see above). Robust-performing models are the best mitigation, together with the mitigations for poisoning and limiting access to model parameters. Alternative name: evasion attacks.
